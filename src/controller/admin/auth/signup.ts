...
export const Signup = async(req: Request, res: Response):Promise<any>=>{
    ...
    if(existingAdmin){
        return res.status(401).json({message:"Admin is already exist"})
    }

    const hashedPassword = await bcrypt.hash(password,10);
    const admin = Admin.create({ firstname, lastname, email, phone, password:hashedPassword });

    await sendEmail(email, "Welcome to Crewplay",`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
